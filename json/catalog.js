...
const arrayMethods = [ /*массив, хранящий конфгурацию кнопок*/
  ...
  { 
    name: "sort (by rating)", 
    method: () => [...services].sort((a, b) => b.rating - a.rating) /* => слева парметры, справа - тело фцнкции, service.filter - то, что функция вернет при вызове, результат нажатия*/
...
function renderCards(data, showDiscount = false) {
 cardsContainer.innerHTML = ''; //удаляем все содержимое5 внутри, чтобы при новой отрисовке карточки не дублировалист
